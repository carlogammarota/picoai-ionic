<template>
  <ion-page>
    <Header />
    <ion-content :fullscreen="true" color="dark">
      <section class="live-banner ion-padding">
        <ion-card mode="ios" color="medium-dark">
          <ion-img 
            src="https://images.unsplash.com/photo-1514525253161-7a46d19cd819" 
            alt="DJ en cabina"
            loading="lazy"
          />
          
          <ion-card-header>
            <ion-card-title class="ion-text-center">
              <ion-text color="light">Transmisión en Vivo</ion-text>
            </ion-card-title>
            <ion-card-subtitle class="ion-text-center">
              <ion-text color="tertiary">Ahora sonando: DJ Aura - Session #124</ion-text>
            </ion-card-subtitle>
          </ion-card-header>
          
          <ion-card-content class="ion-text-center">
            <ion-button 
              expand="block" 
              shape="round" 
              color="primary"
              aria-label="Escuchar transmisión en vivo"
            >
              <ion-icon slot="start" :icon="playCircleOutline" />
              Escuchar en Vivo
            </ion-button>
            <div class="tags-container">
              <ion-chip color="primary" outline>
                <ion-label>#Electrónica</ion-label>
              </ion-chip>
              <ion-chip color="tertiary" outline>
                <ion-label>#House</ion-label>
              </ion-chip>
            </div>
          </ion-card-content>
        </ion-card>
      </section>

      <section class="featured-shows ion-padding">
        <h2 class="ion-text-center">
          <ion-text color="light">Programas Destacados</ion-text>
        </h2>
        
        <ion-grid>
          <ion-row>
            <ion-col 
              v-for="show in featuredShows" 
              :key="show.id"
              size="12" 
              size-md="4"
            >
              <ion-card mode="ios" color="medium-dark">
                <ion-badge v-if="show.live" color="danger">En Vivo</ion-badge>
                <ion-img 
                  :src="show.image" 
                  :alt="show.title"
                  loading="lazy"
                />
                
                <ion-card-header>
                  <ion-card-title>
                    <ion-text color="light">{{ show.title }}</ion-text>
                  </ion-card-title>
                  <ion-card-subtitle>
                    <ion-text color="tertiary">{{ show.time }}</ion-text>
                  </ion-card-subtitle>
                </ion-card-header>
                
                <ion-card-content>
                  <ion-button 
                    expand="block" 
                    fill="outline" 
                    color="tertiary" 
                    size="small"
                    :aria-label="`Ver más sobre ${show.title}`"
                  >
                    Ver más
                    <ion-icon slot="end" :icon="arrowForwardOutline" />
                  </ion-button>
                </ion-card-content>
              </ion-card>
            </ion-col>
          </ion-row>
        </ion-grid>
      </section>

      <section class="hosts-section ion-padding">
        <h2 class="ion-text-center">
          <ion-text color="light">Nuestros Conductores</ion-text>
        </h2>
        
        <ion-grid>
          <ion-row class="ion-justify-content-center">
            <ion-col 
              v-for="host in hosts" 
              :key="host.id"
              size="6" 
              size-md="3"
            >
              <div class="ion-text-center">
                <ion-avatar class="host-avatar">
                  <img 
                    :src="host.avatar" 
                    :alt="`Foto de ${host.name}`"
                    loading="lazy"
                  />
                </ion-avatar>
                <h3>
                  <ion-text color="light">{{ host.name }}</ion-text>
                </h3>
                <ion-chip color="medium" outline>
                  <ion-label>{{ host.show }}</ion-label>
                </ion-chip>
              </div>
            </ion-col>
          </ion-row>
        </ion-grid>
      </section>

      <section class="social-section ion-padding">
        <h2 class="ion-text-center">
          <ion-text color="light">Síguenos en Redes</ion-text>
        </h2>
        
        <div class="ion-text-center ion-padding">
          <ion-button 
            v-for="(social, index) in socialLinks" 
            :key="index"
            fill="clear" 
            :color="social.color" 
            class="social-button"
            :aria-label="`Ir a ${social.name}`"
          >
            <ion-icon slot="icon-only" :icon="social.icon" size="large" />
          </ion-button>
        </div>
      </section>
    </ion-content>

    <ion-tab-bar slot="bottom" color="dark" mode="ios">
      <ion-tab-button 
        v-for="tab in tabs" 
        :key="tab.name"
        :tab="tab.name" 
        :href="tab.route"
        :selected="tab.selected"
      >
        <ion-icon :icon="tab.icon" />
        <ion-label>{{ tab.label }}</ion-label>
      </ion-tab-button>
    </ion-tab-bar>
  </ion-page>
</template>

<script setup lang="ts">
import { 
  IonPage, IonContent, IonButton, IonIcon, IonText, IonCard, 
  IonCardHeader, IonCardTitle, IonCardSubtitle, IonCardContent, 
  IonImg, IonChip, IonLabel, IonBadge, IonGrid, IonRow, IonCol, 
  IonAvatar, IonTabBar, IonTabButton 
} from '@ionic/vue';
import { 
  playCircleOutline, arrowForwardOutline, homeOutline, 
  listOutline, radioOutline, mailOutline, logoInstagram,
  logoTiktok, logoPatreon, logoSpotify
} from 'ionicons/icons';
import { ref } from 'vue';
import Header from '@/componentes/Header.vue';

const featuredShows = ref([
  {
    id: 1,
    title: "Noche Electrónica",
    time: "Lunes 20:00 - 22:00",
    image: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4",
    live: false
  },
  {
    id: 2,
    title: "Indie Sessions",
    time: "Miércoles 18:00 - 20:00",
    image: "https://images.unsplash.com/photo-1470225620780-dba8ba36b745",
    live: true
  },
  {
    id: 3,
    title: "Jazz Lounge",
    time: "Viernes 21:00 - 23:00",
    image: "https://images.unsplash.com/photo-1415201364774-f6f0bb35f28f",
    live: false
  }
]);

const hosts = ref([
  {
    id: 1,
    name: "DJ Aura",
    avatar: "https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad",
    show: "Noche Electrónica"
  },
  {
    id: 2,
    name: "Marta Sol",
    avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330",
    show: "Indie Sessions"
  },
  {
    id: 3,
    name: "Carlos Jazz",
    avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d",
    show: "Jazz Lounge"
  },
  {
    id: 4,
    name: "Luna Beat",
    avatar: "https://images.unsplash.com/photo-1517841905240-472988babdf9",
    show: "Urban Vibes"
  }
]);

const socialLinks = ref([
  { name: 'Instagram', icon: logoInstagram, color: 'instagram' },
  { name: 'TikTok', icon: logoTiktok, color: 'tiktok' },
  { name: 'Patreon', icon: logoPatreon, color: 'patreon' },
  { name: 'Spotify', icon: logoSpotify, color: 'spotify' }
]);

const tabs = ref([
  { name: 'inicio', route: '/inicio', icon: homeOutline, label: 'Inicio', selected: true },
  { name: 'programas', route: '/programas', icon: listOutline, label: 'Programas' },
  { name: 'en-vivo', route: '/en-vivo', icon: radioOutline, label: 'En Vivo' },
  { name: 'contacto', route: '/contacto', icon: mailOutline, label: 'Contacto' }
]);
</script>

<style scoped>
.live-banner {
  background: linear-gradient(to bottom, var(--ion-color-dark) 0%, var(--ion-color-medium) 100%);
}

.tags-container {
  display: flex;
  justify-content: center;
  gap: 8px;
  margin-top: 12px;
}

.host-avatar {
  width: 80px;
  height: 80px;
  margin: 0 auto 10px;
  border: 2px solid var(--ion-color-tertiary);
}

.social-button {
  --border-radius: 50%;
  margin: 0 5px;
}

ion-card {
  transition: transform 0.2s ease;
  will-change: transform;
}

ion-card:active {
  transform: scale(0.98);
}

ion-badge {
  position: absolute;
  top: 10px;
  right: 10px;
  z-index: 10;
}

ion-tab-bar {
  --background: var(--ion-color-dark);
  --border: none;
}

ion-tab-button {
  --color: var(--ion-color-medium);
  --color-selected: var(--ion-color-primary);
}

h2, h3, h4 {
  margin: 0;
  font-weight: 500;
}
</style>